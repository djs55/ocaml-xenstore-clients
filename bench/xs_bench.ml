...
let getdomainpath domid client =
	immediate client (fun xs -> getdomainpath xs domid) >>= fun dom_path ->
	return (prefix ^ dom_path)

let readdir d client =
	Lwt.catch
		(fun () -> immediate client (fun xs -> directory xs d))
	  (function | Xs_protocol.Enoent _ -> return [] | e -> Lwt.fail e)

let read_opt path xs =
	Lwt.catch
		(fun () -> read xs path >>= fun x -> return (Some x))
	  (function | Xs_protocol.Enoent _ -> return None | e -> Lwt.fail e)
...
let list_frontends domid client =
	...
	list_kinds frontend_dir client >>= fun kinds ->

	Lwt_list.map_s
		(fun k ->
			let dir = Printf.sprintf "%s/%s" frontend_dir (string_of_kind k) in
			readdir dir client >|= List.map parse_int >|= to_list >>= fun devids ->
			Lwt_list.map_s
				(fun devid ->
              ...
              immediate client
                (fun xs ->
                  read xs (Printf.sprintf "%s/%d/backend" dir devid) >>= fun x ->
...
let backend_path_of_device (x: device) client =
	getdomainpath x.backend.domid client >>= fun dom_path ->
...
let frontend_error_path_of_device (x: device) client =
	getdomainpath x.frontend.domid client >>= fun dom_path ->
...
let hard_shutdown_request (x: device) client =
	backend_path_of_device x client >>= fun backend_path ->
	frontend_path_of_device x client >>= fun frontend_path ->
	let online_path = backend_path ^ "/online" in
	immediate client
		(fun xs ->
			write xs online_path "0" >>= fun () ->
			rm xs frontend_path >>= fun () ->
			return ()
		)
...
let rm_device_state (x: device) client =
	immediate client
		(fun xs ->
			frontend_path_of_device x client >>= fun fe ->
			backend_path_of_device x client >>= fun be ->
			backend_error_path_of_device x client >>= fun ber ->
			frontend_error_path_of_device x client >>= fun fer ->
			Lwt_list.iter_s (rm xs) [ fe; be; ber; Filename.dirname fer ]
		)

let hard_shutdown device client =
	hard_shutdown_request device client >>= fun () ->
	rm_device_state device client >>= fun () ->
	return ()
...
let add device client =
	...
	frontend_path_of_device device client >>= fun frontend_path ->
	backend_path_of_device device client >>= fun backend_path ->
	let hotplug_path = get_hotplug_path device in
	let private_data_path = get_private_data_path_of_device device in
	transaction client
		(fun xs ->
			exists (Printf.sprintf "/local/domain/%d/vm" device.backend.domid) xs >>= fun _ ->
			exists frontend_path xs >>= fun _ ->
			Lwt.catch (fun () -> rm xs frontend_path) (fun _ -> return ()) >>= fun () ->
			Lwt.catch (fun () -> rm xs backend_path) (fun _ -> return ()) >>= fun () ->
			...
			(* CA-16259: don't clear the 'hotplug_path' because this is where we
			   ...
			   one per PV .iso *)
			mkdir xs frontend_path >>= fun () ->
			setperms xs frontend_path (Xs_protocol.ACL.({owner = device.frontend.domid; other = NONE; acl = [ device.backend.domid, READ ]})) >>= fun () ->
			mkdir xs backend_path >>= fun () ->
			setperms xs backend_path (Xs_protocol.ACL.({owner = device.backend.domid; other = NONE; acl = [ device.frontend.domid, READ ]})) >>= fun () ->
			mkdir xs hotplug_path >>= fun () ->
			setperms xs hotplug_path (Xs_protocol.ACL.({owner = device.backend.domid; other = NONE; acl = []})) >>= fun () ->
			Lwt_list.iter_s (fun (x, y) -> write xs (frontend_path ^ "/" ^ x) y)
		        (("backend", backend_path) :: frontend_list) >>= fun () ->
			Lwt_list.iter_s (fun (x, y) -> write xs (backend_path ^ "/" ^ x) y)
				(("frontend", frontend_path) :: backend_list) >>= fun () ->
			mkdir xs private_data_path >>= fun () ->
			setperms xs private_data_path (Xs_protocol.ACL.({owner = device.backend.domid; other = NONE; acl = []})) >>= fun () ->
			Lwt_list.iter_s (fun (x, y) -> write xs (private_data_path ^ "/" ^ x) y)
				(("backend-kind", string_of_kind device.backend.kind) ::
					("backend-id", string_of_int device.backend.domid) :: private_list) >>= fun () ->
			return ()
		) >>= fun () ->
	return ()
end
...
let make domid client =
  ...
  transaction client
    (fun xs ->
      ...
      Lwt.catch (fun () -> rm xs dom_path) (fun _ -> return ()) >>= fun () ->
      mkdir xs dom_path >>= fun () ->
      setperms xs dom_path roperm >>= fun () ->
      (* The /vm path needs to be shared over a localhost migrate *)
      immediate client (exists vm_path) >>= fun vm_exists ->
      if not vm_exists then begin
        mkdir xs vm_path >>= fun () ->
        setperms xs vm_path roperm >>= fun () ->
        write xs (vm_path ^ "/uuid") uuid >>= fun () ->
        write xs (vm_path ^ "/name") name >>= fun () ->
        return ()
      end else return () >>= fun () ->
      write xs (Printf.sprintf "%s/domains/%d" vm_path domid) dom_path >>= fun () ->

      mkdir xs vss_path >>= fun () ->
      setperms xs vss_path rwperm >>= fun () ->

      write xs (dom_path ^ "/vm") vm_path >>= fun () ->
      write xs (dom_path ^ "/vss") vss_path >>= fun () ->
      write xs (dom_path ^ "/name") name >>= fun () ->
      ...
      ) [ "device"; "error"; "drivers"; "control"; "attr"; "data"; "messages"; "vm-data" ] >>= fun () ->
      return ()
  ) >>= fun () ->
	immediate client
		(fun xs ->

			Lwt_list.iter_s (fun (x, y) -> write xs (dom_path ^ "/" ^ x) y) xsdata >>= fun () ->

			Lwt_list.iter_s (fun (x, y) -> write xs (dom_path ^ "/platform/" ^ x) y) platformdata >>= fun () ->
			Lwt_list.iter_s (fun (x, y) -> write xs (dom_path ^ "/bios-strings/" ^ x) y) bios_strings >>= fun () ->
			...
			write xs (dom_path ^ "/action-request") "poweroff" >>= fun () ->

			write xs (dom_path ^ "/control/platform-feature-multiprocessor-suspend") "1" >>= fun () ->
			...
			write xs (dom_path ^ "/unique-domain-id") uuid >>= fun () ->
			return ()
		) >>= fun () ->
	return ()
...
let shutdown domid req client =
	...
	control_shutdown domid client >>= fun path ->
	getdomainpath domid client >>= fun dom_path ->
	transaction client
		(fun xs ->
			(* Fail if the directory has been deleted *)
			immediate client (exists dom_path) >>= fun domain_exists ->
			if domain_exists then begin
				write xs path reason >>= fun () ->
				return true
...
let destroy domid client =
	...
	Device.list_frontends domid client >>= fun all_devices ->
	...
	immediate client (read_opt (dom_path ^ "/vm")) >>= fun vm_path ->
	immediate client (read_opt (dom_path ^ "/vss")) >>= fun vss_path ->
	begin match vm_path with
		| Some vm_path ->
			immediate client
				(fun xs ->
					rm xs (vm_path ^ "/domains/" ^ (string_of_int domid)) >>= fun () ->
					readdir (vm_path ^ "/domains") client >>= fun domains ->
	...
	immediate client (fun xs -> rm xs dom_path) >>= fun () ->
	getdomainpath 0 client >|= (fun x -> x ^ "/backend") >>= fun backend_path ->
	readdir backend_path client >>= fun all_backend_types ->
	Lwt_list.iter_s
		(fun ty ->
			immediate client (fun xs -> rm xs (Printf.sprintf "%s/%s/%d" backend_path ty domid))
		) all_backend_types >>= fun () ->
	return ()
end

let vm_shutdown domid client =
	Domain.shutdown domid () client >>= fun _ ->
...
let query m n client =
    ...
    (fun _ ->
      Lwt_list.iter_p
      (fun domid ->
        immediate client (fun xs -> read xs (Printf.sprintf "%s/local/domain/%d/name" prefix domid) >>= fun _ -> return ())
      ) (between 0 n)
    ) (between 0 m) >>= fun () ->
	Lwt_list.iter_s
    (fun domid -> 
